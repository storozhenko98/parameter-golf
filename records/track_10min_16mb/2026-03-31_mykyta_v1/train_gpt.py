...
def _byte_shuffle(data: bytes, stride: int = 2) -> bytes:
    """Transpose bytes in groups of `stride` to improve compression (like blosc shuffle)."""
    n = len(data)
    pad = (stride - n % stride) % stride
    arr = bytearray(data) + bytearray(pad)
    total = len(arr)
    block = total // stride
    out = bytearray(total)
    for s in range(stride):
        for i in range(block):
            out[s * block + i] = arr[i * stride + s]
    return bytes(out[:n + pad]), pad

def _byte_unshuffle(data: bytes, pad: int, stride: int = 2) -> bytes:
    ...
    for s in range(stride):
        for i in range(block):
            out[i * stride + s] = data[s * block + i]
    return bytes(out[:total - pad])
import numpy as np
...
class Hyperparameters:
    ...
    mlp_mult = float(os.environ.get("MLP_MULT", 3.5))  # wider MLP, funded by int5 savings
    ...
    late_matrix_lr = float(os.environ.get("LATE_MATRIX_LR", 0.030))  # higher LR for late layers
    late_layer_start = int(os.environ.get("LATE_LAYER_START", 6))  # layers >= this get late LR
...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float, momentum: float, backend_steps: int,
        ...
        self._built = False
        # Per-slice LR multipliers: {param_id: Tensor[B] of multipliers}
        self._slice_lr_muls = slice_lr_muls or {}
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if prev_ag_handle is not None:
                    ...
                    slm = self._slice_lr_muls.get(id(pp))
                    if slm is not None:
                        upd_scaled = upd.to(dtype=pp.dtype) * slm.to(dtype=pp.dtype, device=pp.device).view(-1, *([1]*(pp.ndim-1)))
                        pp.add_(upd_scaled, alpha=-lr * prev_m['scale'])
                    else:
                ...
                if sharded:
                    ...
                    # Apply per-slice LR multiplier if available
                    slm = self._slice_lr_muls.get(id(p))
                    if slm is not None:
                        upd = update.to(dtype=p.dtype) * slm.to(dtype=p.dtype, device=p.device).view(-1, *([1]*(p.ndim-1)))
                        p.add_(upd, alpha=-lr * m['scale'])
                    else:
...
class CastedLinear(nn.Linear):
    ...
    _qat_alpha: float = 1.0  # soft-round temperature (ramps 1->16 over training)
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            # Soft-round: differentiable sigmoid rounding with temperature alpha
            alpha = CastedLinear._qat_alpha
            floored = scaled.floor()
            frac = scaled - floored
            soft_rounded = floored + torch.sigmoid(alpha * (frac - 0.5))
            w_q = (torch.clamp(soft_rounded, -32, 31) * scale[:, None]).to(x.dtype)
            w = w_q  # soft-round provides real gradients, no STE needed
...
def eval_val_sliding_slot(
    ...
    """Sliding window eval with SLOT: per-batch delta optimization on hidden states."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
    ...
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    # Get model's logit projection parameters
    softcap = base_model.logit_softcap
    tie = base_model.tie_embeddings
    for bi in range(0, len(my_windows), batch_seqs):
        batch_ws = my_windows[bi:bi + batch_seqs]
        bsz = len(batch_ws)
        x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        ...
        for i, ws in enumerate(batch_ws):
            ...
            x_batch[i, :wlen] = chunk[:-1]
            y_batch[i, :wlen] = chunk[1:]
        # Phase 1: Forward pass to get hidden states (no grad)
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            n = base_model.num_layers
            x = base_model.tok_emb(x_batch)
            if base_model.bigram is not None:
                x = x + base_model.bigram(x_batch)
            x = F.rms_norm(x, (x.size(-1),))
            ...
            skips: list[Tensor] = []
            ve_cache: dict = {}
            for li in range(base_model.num_encoder_layers):
                ve = base_model._get_ve(li, x_batch, ve_cache)
                x, raw_v = base_model.blocks[li](x, x0,
                    base_model.qo_bank[li], base_model.kv_bank[li], base_model.kv_bank[n+li],
                    base_model.qo_bank[n+li], base_model.mlp_up_bank[li], base_model.mlp_down_bank[li],
                    v_embed=ve, v0=v0)
                if v0 is None and raw_v is not None:
                    v0 = raw_v
                skips.append(x)
            for li in range(base_model.num_decoder_layers):
                bii = base_model.num_encoder_layers + li
                if skips:
                    x = x + base_model.skip_weights[li].to(dtype=x.dtype)[None, None, :] * skips.pop()
                ve = base_model._get_ve(bii, x_batch, ve_cache)
                x, _ = base_model.blocks[bii](x, x0,
                    base_model.qo_bank[bii], base_model.kv_bank[bii], base_model.kv_bank[n+bii],
                    base_model.qo_bank[n+bii], base_model.mlp_up_bank[bii], base_model.mlp_down_bank[bii],
                    v_embed=ve, v0=v0)
            hidden = base_model.final_norm(x).detach()  # [bsz, seq_len, dim]
        # Phase 2: Optimize per-batch delta on hidden states
        delta = torch.zeros(1, 1, hidden.size(-1), device=device, dtype=torch.float32, requires_grad=True)
        opt_delta = torch.optim.AdamW([delta], lr=slot_lr, weight_decay=0.0)
        if tie:
            proj_w = base_model.tok_emb.weight.detach()
        else:
            proj_w = base_model.lm_head.weight.detach()
        for _slot_step in range(slot_steps):
            opt_delta.zero_grad()
            h = hidden.float() + delta
            logits_proj = F.linear(h.reshape(-1, h.size(-1)), proj_w.float())
            logits = softcap * torch.tanh(logits_proj / softcap)
            slot_loss = F.cross_entropy(logits, y_batch.reshape(-1), reduction="mean")
            slot_loss.backward()
            opt_delta.step()
        # Phase 3: Score with optimized delta (no grad)
        with torch.no_grad():
            h = hidden.float() + delta.detach()
            logits_proj = F.linear(h.reshape(-1, h.size(-1)), proj_w.float())
            logits = softcap * torch.tanh(logits_proj / softcap)
            nll = F.cross_entropy(
                logits, y_batch.reshape(-1), reduction="none"
            ).reshape(bsz, seq_len)
        for i, ws in enumerate(batch_ws):
            wlen = wlens[i]
            s = 0 if ws == 0 else max(wlen - stride, 0)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            cr = 15 if cat == "mlp" else 31  # int5 for MLP, int6 for attention
            ...
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{'5' if cr == 15 else '6'}"}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def main() -> None:
    ...
    # Build per-slice LR multipliers for split early/late LR
    slice_lr_muls = {}
    if args.late_matrix_lr != args.matrix_lr:
        n = args.num_layers
        late_mul = args.late_matrix_lr / args.matrix_lr
        for bank_param, bank_size in [
            (base_model.qo_bank, 2 * n),  # Q[0..N-1], Out[N..2N-1]
            (base_model.kv_bank, 2 * n),   # K[0..N-1], V[N..2N-1]
            (base_model.mlp_up_bank, n),
            (base_model.mlp_down_bank, n),
        ]:
            muls = torch.ones(bank_size, dtype=torch.float32)
            for i in range(bank_size):
                layer_idx = i % n  # maps Q/K/Up/Down slice i -> layer i, Out/V slice N+i -> layer i
                if layer_idx >= args.late_layer_start:
                    muls[i] = late_mul
            slice_lr_muls[id(bank_param)] = muls
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            CastedLinear._qat_alpha = 1.0
            CastedLinear._qat_start_step = step
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} (soft-round alpha ramp)")
        if CastedLinear._qat_enabled:
            # Ramp alpha from 1 (smooth) to 16 (near-hard) over 500 steps
            qat_progress = min((step - getattr(CastedLinear, '_qat_start_step', step)) / 500.0, 1.0)
    ...
    if ones_info:
        ...
        def _try_prune(n):
            ...
            raw = buf.getvalue()
            sz_lzma = len(lzma.compress(raw, preset=9))
            sz = sz_lzma
            if _HAS_BROTLI:
                shuf, sp = _byte_shuffle(raw, stride=2)
                sz_brotli = len(brotli.compress(shuf, quality=11)) + 4
                sz = min(sz, sz_brotli)
            return sz + code_bytes_est, tmp
    ...
    # Try both Brotli-11 with byte-shuffle and LZMA-9, keep smaller
    quant_blob_lzma = lzma.compress(quant_raw, preset=9)
    use_brotli = False
    if _HAS_BROTLI:
        shuffled, shuf_pad = _byte_shuffle(quant_raw, stride=2)
        quant_blob_brotli = brotli.compress(shuffled, quality=11)
        # Prepend 4-byte pad length header for decompression
        header = shuf_pad.to_bytes(4, 'little') if isinstance(shuf_pad, int) else int(shuf_pad).to_bytes(4, 'little')
        quant_blob_brotli = header + quant_blob_brotli
        if len(quant_blob_brotli) < len(quant_blob_lzma):
            quant_blob = quant_blob_brotli
            use_brotli = True
        else:
            quant_blob = quant_blob_lzma
    else:
        quant_blob = quant_blob_lzma
    compressor_name = "brotli11+shuffle" if use_brotli else "lzma9"
    if master_process:
        with open("final_model.int6.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int6+{compressor_name}: {quant_file_bytes} bytes (lzma={len(quant_blob_lzma)})")
        log0(f"Total submission size int6+{compressor_name}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.int6.ptz", "rb") as f:
        quant_blob_disk = f.read()
    # Decompress: detect format by trying brotli first (has 4-byte header)
    try:
        if _HAS_BROTLI and len(quant_blob_disk) > 4:
            shuf_pad = int.from_bytes(quant_blob_disk[:4], 'little')
            if shuf_pad < 16:  # sanity check -- pad is always 0 or 1 for stride=2
                decompressed_shuffled = brotli.decompress(quant_blob_disk[4:])
                quant_raw_loaded = _byte_unshuffle(decompressed_shuffled, shuf_pad, stride=2)
            else:
                raise ValueError("not brotli")
        else:
            raise ValueError("no brotli")
    except Exception:
        quant_raw_loaded = lzma.decompress(quant_blob_disk)
    quant_state = torch.load(
        io.BytesIO(quant_raw_loaded),
        map_location="cpu",
    )
    ...
    # SLOT eval: per-batch delta optimization on hidden states
    if args.slot_enabled and args.eval_stride > 0:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_slot_sliding val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
            f"slot_steps:{args.slot_steps} slot_lr:{args.slot_lr} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms"
        )
        log0(f"final_slot_sliding_exact val_loss:{slot_val_loss:.8f} val_bpb:{slot_val_bpb:.8f}")
    if distributed:
