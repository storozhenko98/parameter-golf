...
class Hyperparameters:
    ...
    trigram_enabled = bool(int(os.environ.get("TRIGRAM", "1")))  # TrigramHash enabled
...
def main() -> None:
    ...
    if ones_info:
        ...
        def _try_prune(n):
            ...
            raw = buf.getvalue()
            sz = len(lzma.compress(raw, preset=9))
            if _HAS_BROTLI:
                shuf, sp = _byte_shuffle(raw, stride=2)
                sz = min(sz, len(brotli.compress(shuf, quality=11)) + 4)
    ...
    if _HAS_BROTLI:
        ...
        quant_blob_brotli = shuf_pad.to_bytes(4, 'little') + brotli.compress(shuffled, quality=11)
    ...
    try:
        if _HAS_BROTLI and len(quant_blob_disk) > 4:
            ...
            if shuf_pad < 16:
                quant_raw_loaded = _byte_unshuffle(brotli.decompress(quant_blob_disk[4:]), shuf_pad, stride=2)
            else:
